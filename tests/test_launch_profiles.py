...
@mock.patch("pathtree.utils.launcher.PlatformLauncher.launch_process")
@mock.patch("pathtree.utils.launcher.PlatformLauncher.launch_in_terminal")
def test_service_terminal_mode_override(
    mock_launch_in_terminal,
    mock_launch_process,
    session,
    node_service,
    ...
    mock_launch_process.return_value = ProcessLaunchResult(success=True, pid=123)
    mock_launch_in_terminal.return_value = ProcessLaunchResult(success=True, pid=456)
    ...
    # 1. Profile default terminal_mode is "inherit"
    profile = launch_profile_service.create_profile(
        name="Run Profile",
        target_node_id=script_node.id,
        arguments=["--flag", "val"],
    ...
    # 2. Run with default (inherit), should call launch_process
    res = launch_profile_service.execute_profile(profile.id)
    assert res.success is True
    assert res.pid == 123
    mock_launch_process.assert_called_once()
    mock_launch_in_terminal.assert_not_called()

    mock_launch_process.reset_mock()
    mock_launch_in_terminal.reset_mock()

    # 3. Override with "new_terminal", should call launch_in_terminal
    res = launch_profile_service.execute_profile(
        profile.id, terminal_mode_override="new_terminal"
    )
    assert res.success is True
    assert res.pid == 456
    mock_launch_process.assert_not_called()
    mock_launch_in_terminal.assert_called_once()

    mock_launch_process.reset_mock()
    mock_launch_in_terminal.reset_mock()

    # 4. Create another profile with terminal_mode "new_terminal"
    profile2 = launch_profile_service.create_profile(
        name="Run Profile 2",
        target_node_id=script_node.id,
        arguments=["--flag", "val"],
    ...
    # 5. Run profile2 with override "inherit", should call launch_process
    res = launch_profile_service.execute_profile(
        profile2.id, terminal_mode_override="inherit"
    )
    assert res.success is True
    assert res.pid == 123
    mock_launch_process.assert_called_once()
